...
impl RustFileGenerator {
    ...
    pub fn generate_file_rtlolaout_service(
        &self,
        ...
        if let Some((rtlola_package, _, members_type_and_name)) = rtlolaout_service {
            let rtlola_package = rtlola_package.replace('/', "::");
            // File that is generated
            let file_location = format!("{}/output/rtlolaout_service.rs", self.dest_path);
                ...
                match ty.as_str() {
                    "bool" => {
                        ...
                        value_change.push_str(&format!(
                            "{i} => self.latest_response.{name} = value_as_bool(v)?,"
                        ));
                    }
                    "float32" => {
                        default_values.push_str(&format!("{name}: 0.0,"));
                        value_change.push_str(&format!(
                            "{i} => self.latest_response.{name} = value_as_f32(v)?,"
                        ));
                    }
                    "float64" => {
                        default_values.push_str(&format!("{name}: 0.0,"));
                        value_change.push_str(&format!(
                            "{i} => self.latest_response.{name} = value_as_f64(v)?,"
                        ));
                    }
                    "int8" => {
                        default_values.push_str(&format!("{name}: 0,"));
                        value_change.push_str(&format!(
                            "{i} => self.latest_response.{name} = value_as_i8(v)?,"
                        ));
                    }
                    "uint8" => {
                        default_values.push_str(&format!("{name}: 0,"));
                        value_change.push_str(&format!(
                            "{i} => self.latest_response.{name} = value_as_u8(v)?,"
                        ));
                    }
                    "int16" => {
                        default_values.push_str(&format!("{name}: 0,"));
                        value_change.push_str(&format!(
                            "{i} => self.latest_response.{name} = value_as_i16(v)?,"
                        ));
                    }
                    "uint16" => {
                        default_values.push_str(&format!("{name}: 0,"));
                        value_change.push_str(&format!(
                            "{i} => self.latest_response.{name} = value_as_u16(v)?,"
                        ));
                    }
                    "int32" => {
                        default_values.push_str(&format!("{name}: 0,"));
                        value_change.push_str(&format!(
                            "{i} => self.latest_response.{name} = value_as_i32(v)?,"
                        ));
                    }
                    "uint32" => {
                        default_values.push_str(&format!("{name}: 0,"));
                        value_change.push_str(&format!(
                            "{i} => self.latest_response.{name} = value_as_u32(v)?,"
                        ));
                    }
                    ...
                    "int64" => {
                        default_values.push_str(&format!("{name}: 0,"));
                        value_change.push_str(&format!(
                            "{i} => self.latest_response.{name} = value_as_i64(v)?,"
                        ));
                    }
                    ...
                    "uint64" => {
                        default_values.push_str(&format!("{name}: 0,"));
                        value_change.push_str(&format!(
                            "{i} => self.latest_response.{name} = value_as_u64(v)?,"
                        ));
                    }
                    _ => {
                        panic!(
            ...
            let absolute_path_to_input_template =
                crate_root.join("build_src/templates/t_rtlolaout_service.rs");
